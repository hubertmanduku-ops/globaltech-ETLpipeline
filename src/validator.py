...
#
# CONTEXT: We just ran Module 03 and extracted employee data from
# GlobalTech's database into raw-data.csv.
...
#   4. check_numeric_ranges() → are there impossible values (negative salary)?
...
while not (_root / "config.py").exists() and _root != _root.parent:
    # move up one level: src/ → teaching-project/ → module-05/ → ...
    _root = _root.parent
