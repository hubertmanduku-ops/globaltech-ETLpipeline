...
class DataTransformer:
    ...
    def fill_nulls(self) -> "DataTransformer":
        ...
        salary, should we remove those 23 people from all future analysis?
        That seems unfair — especially if they all happen to work in one department.

        WHY NOT FILL EVERYTHING WITH ZERO?
        ─────────────────────────────────────
        Filling salary nulls with 0 would make the average salary appear
        dramatically lower than it really is. A salary of zero is not
    ...
    def fix_types(self) -> "DataTransformer":
        ...
        THE SYMPTOM:
        ─────────────
        salary = "92000" (a string with quotes)
...
if __name__ == "__main__":
    ...
    test_df = pd.DataFrame({
        ...
        "salary":      [50000, None, 70000, -500, 90000, 70000],  # null + negative
